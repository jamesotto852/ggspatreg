...
vgm_fit <- gstat::fit.variogram(vgm, model = gstat::vgm(NA, "Exp", NA, NA))
vgm_fit <- gstat::fit.variogram(vgm, model = gstat::vgm("Exp"))
...
```{r loading_functions}
list.files(here::here("R"), full.names = TRUE) |> walk(source)
```
...
ggplot(df, aes(x, y)) +
  geom_krige(aes(z = z)) +
  geom_point(aes(fill = z), shape = 21, size = 1.25) +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE) +
  scale_fill_viridis_c() +
  guides(size = guide_legend(reverse = TRUE))
```
...
p_low <- ggplot(df, aes(x, y, z = z)) +
  geom_krige(nx = 50, ny = 50) +
  scale_fill_viridis_c() +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE) 

p_high <- ggplot(df, aes(x, y, z = z)) +
  geom_krige(nx = 200, ny = 200) +
  scale_fill_viridis_c() +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE)
...
p_contour_lower <- ggplot(df, aes(x, y, z = z)) +
  geom_krige_contour(nx = 30, ny = 30, bins = 5) +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE)

p_contour_filled_lower <- ggplot(df, aes(x, y, z = z)) +
  geom_krige_contour_filled(nx = 30, ny = 30, bins = 5) +
  scale_fill_viridis_d() +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE)
...
```{r simulating_UK}
set.seed(1)
data <- geoR::grf(grid = dom, cov.model = "exponential", cov.pars = c(10, 20), nugget = .5)$data |>
  (\(z) tibble(z = z))()
...
In order to account for nonconstant mean, we specify the `formula` argument and estimate with universal kriging!

```{r geom_krige_UK_1}
ggplot(df, aes(x, y)) +
  geom_krige(aes(z = z), formula = z ~ x + y) +
  geom_point(aes(fill = z), shape = 21, size = 5, stroke = 1) +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE) +
  scale_fill_viridis_c()
```
...
p_contour_UK <- ggplot(df, aes(x, y)) +
  geom_krige_contour(aes(z = z), formula = z ~ x + y, bins = 5) +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE) +
  scale_fill_viridis_c() 

p_contour_filled_UK <- ggplot(df, aes(x, y)) +
  geom_krige_contour_filled(aes(z = z), formula = z ~ x + y, bins = 5) +
  coord_cartesian(xlim = c(1, 100), ylim = c(1, 100), expand = FALSE) 
