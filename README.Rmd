...
**ggspatreg** extends **ggplot2**, allowing for simple visualization of spatial regression models.
...
It is not available on CRAN, but you can install the development version:
...
```{r ggmap_setup, message = FALSE, results = FALSE}
library("ggmap")
...
```{r ggmap_krige_1, results = FALSE, warning = FALSE}
ggmap(waco_map) +
